/******************************************************************************
TinyMMA8452Q.cpp
TinyMMA8452Q Library Source File

Modified version to use TinyWireM library.
Modified by: qery
https://github.com/qery/TinyMMA8452Q

*******************************************************************************

Originally createdy by:
...
byte MMA8452Q::readRegister(MMA8452Q_Register reg)
{
	...
	TinyWireM.requestFrom(address, (byte) 1); //Ask for 1 byte, once done, bus is released by default
...
void MMA8452Q::readRegisters(MMA8452Q_Register reg, byte *buffer, byte len)
{
	...
	TinyWireM.requestFrom(address, len); //Ask for bytes, once done, bus is released by default

	while(TinyWireM.available() < len); //Hang out until we get the # of bytes we expect

	for(int x = 0 ; x < len ; x++)
		buffer[x] = TinyWireM.read();    
}
